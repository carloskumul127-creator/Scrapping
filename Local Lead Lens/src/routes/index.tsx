...
import {
  ...
  Trash2, EyeOff, MessageCircle, CheckSquare, Square, X,
} from "lucide-react";
    ...
    <div className="min-h-screen px-4 md:px-8 py-6 max-w-[1500px] mx-auto">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Upload className="w-4 h-4 text-azure" /> Cargar Excel/CSV
          </button>
          <button
          ...
          {([
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono transition flex items-center gap-1.5 ${
                waFilter === key ? "bg-emerald/20 text-emerald" : "text-muted-foreground hover:text-foreground"
              }`}
            >
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-mono transition ${
                  sharedFilter === key ? "bg-violet/20 text-violet" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {label} <span className="opacity-60">({fmt(count)})</span>
              </button>
      ...
      <section className="space-y-5">
        {byIndustry.map(({ industry, rows, cities }, i) => {
          const isOpen = openTags[industry] ?? true;
          const accent = ACCENT[i % ACCENT.length];
          const visibleRows = filterRows(rows);
          return (
            <div key={industry} className="glass rounded-2xl overflow-hidden">
              {/* Block header */}
              <button
                ...
                <div className="border-t border-white/5 p-5 space-y-5">
                  {Object.entries(cities)
                    ...
                    .map(([city, crows]) => {
                      ...
                      return (
                        <CityBlock key={city} industry={industry} city={city} rows={cVisible.length ? cVisible : crows} accent={accent} onFocus={setFocused} onOverride={reloadOverrides} />
                      );
                    })}
              ...
              return (
                <tr key={i} className={`border-t border-white/5 transition ${r.hidden ? "opacity-50" : r.shared ? "bg-emerald/[0.04] hover:bg-emerald/[0.08]" : "hover:bg-white/[0.03]"}`}>
                  <td className="px-4 py-2.5 font-medium">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => { onFocus(r); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                ...
                return (
                  <li key={key}>
                    <button
                      ...
                      className={`w-full flex items-center justify-between gap-2 px-3 py-1.5 rounded-lg text-xs transition ${
                        checked ? "bg-white/5" : "hover:bg-white/[0.03] text-muted-foreground"
                      }`}
                    >
