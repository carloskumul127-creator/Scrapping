...
export async function logDownload(entry: {
  ...
  const { error } = await (supabase as any).from("downloads").insert({
...
export async function deleteDownload(id: string) {
  const { error } = await (supabase as any).from("downloads").delete().eq("id", id);
  if (error) console.error("[downloads] delete", error);
}

/* ---------- contact overrides ---------- */
export async function fetchOverrides(): Promise<Record<string, ContactOverride>> {
  const { data, error } = await (supabase as any).from("contact_overrides").select("*");
  if (error) {
